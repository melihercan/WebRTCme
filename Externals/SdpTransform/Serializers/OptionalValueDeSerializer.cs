        ...
        public DeserializationState ReadValue(ReadOnlySpan<byte> data, DeserializationSession session)
        {
            ...
            switch (data[0])
            {
                ...
                case RepeatTimeSerializer.Identifier:
                    session.ParsedValue.RepeatTimes = session.ParsedValue.RepeatTimes ?? new List<RepeatTime>();
                    session.ParsedValue.RepeatTimes.Add(RepeatTimeSerializer.Instance.ReadValue(data));
                    return this.ReadValue;
                case TimezonesSerializer.Identifier:
                    session.ParsedValue.TimeZones = TimezonesSerializer.Instance.ReadValue(data);
                    return this.ReadValue;
