...
namespace WebRTCme
{
    ...
    /// Mirrors the role AndroidSupport plays for the Android binding. Shared by every
    /// platform that uses the SipSorcery binding (Windows and Mac Catalyst).
    /// </summary>
    public static class SipSorcerySupport
    {
        /// <summary>
            ...
            Action<byte[], int, int> onBgraFrame)
        {
            ...
            if (videoTrack is not MediaStreamTrack mediaStreamTrack)
                throw new ArgumentException(
                    $"Track must be created by the SipSorcery binding, got {videoTrack.GetType().FullName}.",
                    nameof(videoTrack));
            ...
            {
                var videoSink = mediaStreamTrack.VideoSink;
                ...
                videoSink.OnVideoSinkDecodedSample += OnDecoded;
                return new Subscription(() => videoSink.OnVideoSinkDecodedSample -= OnDecoded);
            }
            ...
            var videoSource = mediaStreamTrack.VideoSource;
            videoSource.OnVideoSourceRawSample += OnRaw;
            return new Subscription(() => videoSource.OnVideoSourceRawSample -= OnRaw);
        }
