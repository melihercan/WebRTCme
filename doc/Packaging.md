...
(`GeneratePackageOnBuild` is false everywhere) - CI builds and pushes them.
  ...
  WebRTCme              the API, the plug-in and the five bindings, with their native halves
...
Build them with:
  ...
  dotnet pack WebRTCme.Middleware/WebRTCme.Middleware.csproj -c Release -o <dir>
...
- Each folded project is referenced with `PrivateAssets="all"` so pack does not record it as a
  ...
  sets one.
- `PrivateAssets="all"` also stops the types flowing to anything downstream. That is fine for a
  binding, which nothing outside WebRTCme uses, and was the reason the API could not stay a
  separate project: it is compiled into WebRTCme.dll instead.
